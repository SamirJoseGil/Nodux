...
export default function SystemAdminSidebar() {
    ...
    const menuItems = [
        { name: 'Dashboard', path: '/modulo/administracion/dashboard', icon: DashboardIcon },
        { name: 'Usuarios', path: '/modulo/administracion/users', icon: UsersIcon },
        { name: 'Roles y Permisos', path: '/modulo/administracion/roles', icon: SecurityIcon },
        { name: 'Logs del Sistema', path: '/modulo/administracion/logs', icon: LogsIcon },
        { name: 'Configuración', path: '/modulo/administracion/settings', icon: SettingsIcon },
    ];
    ...
    return (
        <>
                    ...
                    {!isCollapsed && (
                        <>
                            <div className="flex-1 min-w-0">
                                <h2 className="text-xl font-bold text-slate-900">Admin del Sistema</h2>
                                <p className="text-sm text-slate-600">Gestión centralizada</p>
                            </div>
                        ...
                        <div className="p-4 border-b border-gray-200 flex-shrink-0">
                            <div className="flex items-center space-x-3">
                                <div className="relative flex-shrink-0">
                                    <div className="bg-red-600 rounded-full h-12 w-12 flex items-center justify-center shadow-sm">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-red-600 font-medium">{user?.role || 'Admin'}</p>
                                </div>
                            </div>
                        </div>
                                    ...
                                    return (
                                        <li key={item.path}>
                                            ...
                                            >
                                                <IconComponent
                                                    size={20}
                                                    className={`mr-3 flex-shrink-0 ${isActive(item.path) ? 'text-red-600' : 'text-slate-500'}`}
                                                />
                                                <span className="font-medium truncate">{item.name}</span>
                                                {isActive(item.path) && (
                                                    <div className="ml-auto w-2 h-2 bg-red-600 rounded-full flex-shrink-0" />
                                                )}
                                            </Link>
                                        </li>
                                    );
            ...
            {isCollapsed && (
                <button
                    onClick={toggleCollapsed}
                    className="fixed top-20 left-4 z-50 p-2 bg-red-600 text-white rounded-lg shadow-lg hover:bg-red-700 transition-colors"
